...
# --- Define tickers (use reliable Yahoo tickers) ---
fintech_tickers = ["PYPL", "ADYEY", "SQ", "WISE.L"]
bank_tickers = ["JPM", "GS", "HSBA.L"]
...
# Drop any columns that are mostly missing (e.g., unavailable tickers)
...
plt.tight_layout()
plt.savefig("assets/correlation_heatmap.png", dpi=160, bbox_inches="tight")
plt.close()
...
# --- Plot rolling Sharpe ratios (6-month) ---
rs_plot = rolling_sharpe[["Fintech Avg", "Bank Avg"]].dropna(how="all")
plt.figure(figsize=(11, 6))
if "Fintech Avg" in rs_plot.columns:
    plt.plot(rs_plot.index, rs_plot["Fintech Avg"], label="Fintech (6-month Sharpe)", color="#2CA02C")
if "Bank Avg" in rs_plot.columns:
    plt.plot(rs_plot.index, rs_plot["Bank Avg"], label="Banks (6-month Sharpe)", color="#1F77B4")
